...
Zhong ShiChang 20210120	  1.0.16 add new function: OPTController_SaveSeqFileToCSV OPTController_LoadSeqFileFromCSV OPTController_CompareSeqTableFromCSV ,change BOOL or bool to int.
CaiBo		   20210703	  1.0.17 add new functions and fix bugs
Zhangzhou Tao  20220222   1.0.19 add new functions
Zhangzhou Tao  20220322   1.1.0 add new functions
*************************************************/

#ifndef __OPTCONTROLER_H__
    #define __OPTCONTROLER_H__

    #ifdef OPT_DLL_EXPORT
...
typedef struct OPTController_DOSoftwareLevelItem {
    int channelIndex;
    int level;
} DOSoftwareLevelItem;
...
/*******************************************************************
	...
	Input(s):
	    ...
	    channelIndex     -the index(es) of the channel(s) to be turned on, range:[0-36] (in decimal form, 0 for all channels)
	Return: 
	    succeed    -OPT_SUCCEED
...
/*******************************************************************
	...
	Input(s):
	    controllerHandle   -the handle of controller
	    channelIndexArray  -an array consists of the indexes of the channels to be turned on, range:[1-36] (in decimal form)
...
/*******************************************************************
	...
	Input(s):
	    controllerHandle -the handle of controller
	    channelIndex     -the index(es) of the channel(s) to be turned off, range:[0-36] (in decimal form, 0 for all channels);
	Return:
	    succeed    -OPT_SUCCEED
...
/*******************************************************************
	...
	Input(s):
	    controllerHandle  -the handle of controller;
	    channelIndexArray -an array consists of the indexes of the channels to be turned off, range:[1-36] (in decimal form);
		length            -the length of the channel index array
	Return: 
	    succeed    -OPT_SUCCEED;
	    failed     -OPT_ERR_TURNOFFCH_FAILED
...
/*******************************************************************
	...
	Input(s):
	    controllerHandle -the handle of controller
	    channelIndex     -the index(es) of the channel(s), range:[0-36] (in decimal form, 0 for all channels)
...
/*******************************************************************
	...
	Input(s):
	controllerHandle -the handle of controller
	channelIndex     -the index of the channel, range: [1-36] (in decimal form)
	Output(s): 
...
/*******************************************************************
	...
	Input(s):
	    controllerHandle  -the handle of controller;
	    channelIndex      -the index(es) of the channel(s), range:[0-36] (in decimal form, 0 for all channels);
	    HBTriggerWidth    -the value of the high brightness trigger pulse width to be set, range:[1-999].
	Return: 
	    succeed    -OPT_SUCCEED
...
/*******************************************************************
	...
	Input:
		controllerHandle  -the handle of controller
		nisResponse        -"non-0" means "need response" while "0" stands for not
	Return: 
	    succeed    -OPT_SUCCEED
	    failed     -OPT_ERR_UNKNOWN
	*****************************************************************/
long DLL_EXPORT OPTController_EnableResponse(OPTController_Handle controllerHandle, int nisResponse);
...
/*******************************************************************
	...
	Input:
	    controllerHandle  -the handle of controller
	    nisCheckSum        -"non-0" means "need checksum" while "0" stands for not
	Return: 
	    succeed    -OPT_SUCCEED
	    failed     -OPT_ERR_UNKNOWN
	*****************************************************************/
long DLL_EXPORT OPTController_EnableCheckSum(OPTController_Handle controllerHandle, int nisCheckSum);
...
/*******************************************************************
	...
	Input:
		controllerHandle   -the handle of controller
		nisSave:            -"non-0" means "need back up" while "0" stands for not
	Return: 
	    succeed    -OPT_SUCCEED
	    failed     -OPT_ERR_UNKNOWN
	*****************************************************************/
long DLL_EXPORT OPTController_EnablePowerOffBackup(OPTController_Handle controllerHandle, int nisSave);
...
/*******************************************************************
	Function: OPTController_ReadIPConfigBySN
	Description: read IP configuration of the controller by SN
	Input:
	SN	                 -the SN of controller
	Output:
	IP                   -the obtained IP address
	subnetMask           -the obtained subnet mask
	defaultGateway       -the obtained default gateway
	...
	Remark:only for Ethernet controllers
	*****************************************************************/
long DLL_EXPORT OPTController_ReadIPConfigBySN(char* SN, char* IP, char* subnetMask, char* defaultGateway);
...
/*******************************************************************
	...
	Input:
		controllerHandle	-the handle of controller
		channelIndex		-the index(es) of the channel(s), range:[0-36] (in decimal form, 0 for all channels);
		current				-the value of the maximum current to be set, range:[1-240].unit 10mA.
	Return: 
		succeed    -OPT_SUCCEED
...
/*******************************************************************
	...
	Input(s):
		...
		maxCurrentArray		 -an array consists of values of the maximum current (and the indexes of corresponding channels) 
							 to be set, range:[1-240],unit 10mA.
...
/*******************************************************************
	...
	Input:
		controllerHandle	-the handle of controller
        channelIndex        -he index(es) of the channel, range:[1-36](in decimal form).
	Output:
...
/*******************************************************************
	...
	Input:
		controllerHandle		-the handle of controller
		channelIndex			-the index(es) of the channel(s), range:[0-36] (in decimal form);
...
/*******************************************************************
	...
	Input:
		controllerHandle		-the handle of controller
		channelIndex			-the index(es) of the channel(s), range:[1-36] (in decimal form);
	Output:
...
/*******************************************************************
	...
	Input:
		controllerHandle		-the handle of controller
		nbDHCP					-"non-0" means "enable DHCP" while "0" stands for disable
	Return: 
		succeed    -OPT_SUCCEED
		failed     -OPT_ERR_SET_DHCP_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_EnableDHCP(OPTController_Handle controllerHandle, int nbDHCP);
...
/****************************************************************
	...
	****************************************************************/
long DLL_EXPORT OPTController_IsConnect(OPTController_Handle controllerHandle);
...
/****************************************************************
	...
	****************************************************************/
long DLL_EXPORT OPTController_GetControllerListOnEthernet(char* snList); //zy
...
/*******************************************************************
	...
	Input:
		controllerHandle		-the handle of controller
		nenable					-"non-0" means "enable keepalive" while "0" stands for disable
	Return: 
		succeed    -OPT_SUCCEED
		failed     -OPT_ERR_ENABLE_KEEPALIVE_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_EnableKeepalive(OPTController_Handle controllerHandle, int nenable);
...
/*******************************************************************
	...
	Input:
	   controllerHandle		-the handle of controller
	    moduleIndex			-the index of the module, range:[1-9],
		                    1 for channel 1 to channel 4;
							2 for channel 5 to channel 8;
							3 for channel 9 to channel 12;
							4 for channel 13 to channel 16;
							5 for channel 17 to channel 20;
							6 for channel 21 to channel 24;
							7 for channel 25 to channel 28;
							8 for channel 29 to channel 32;
                            9 for channel 33 to channel 36;
    Output:
...
/*******************************************************************
	...
	Input:
	   controllerHandle		-the handle of controller   
	   moduleIndex			-the index of the module, range:[1-9],
	                         1 for channel1 to channel 4;
							 2 for channel5 to channel 8;
							 3 for channel9 to channel 12;
							 4 for channel13 to channel 16;
							 5 for channel 17 to channel 20;
							 6 for channel 21 to channel 24;
							 7 for channel 25 to channel 28;
							 8 for channel 29 to channel 32;
	   ...
	   triggerSource        -the trigger source data,range[1-36](
	                         the value of moduleIndex is 1,the range of triggerSource is [1-4];
	                         the value of moduleIndex is 2,the range of triggerSource is [5-8];
							 the value of moduleIndex is 3,the range of triggerSource is [9-12];
							 the value of moduleIndex is 4,the range of triggerSource is [13-16];
							 the value of moduleIndex is 5,the range of triggerSource is [17-20];
							 the value of moduleIndex is 6,the range of triggerSource is [21-24];
							 the value of moduleIndex is 7,the range of triggerSource is [25-28];
							 the value of moduleIndex is 8,the range of triggerSource is [29-32];
							 )
...
/*******************************************************************
	...
	Input:
	   controllerHandle		-the handle of controller
	   moduleIndex			-the index of the channel, range:[1-9] (in decimal form)
	Output:
	    seqCount  		    -the SEQ count,range:[1-64]
...
/*******************************************************************
	Function:OPTController_SaveSeqFile
	Description: Saving the SEQ table data.
	Input:
	    seqCount  		    -the SEQ count,range:[1-64]
	    triggerSource       -the trigger source data
        intensity           -the intensity data
        pulseWidth          -the pulse width data
		filePath			-the filePath of saving the SEQ data file
	Return: 
		succeed    -OPT_SUCCEED
		failed     -OPT_ERR_READSEQTABLEDATA_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_SaveSeqFile(int seqCount, int* triggerSource, int* intensity, int* pulseWidth, char* filePath);

/*******************************************************************
	Function:OPTController_LoadSeqFile
	Description: Loading SEQ table data.
	Input:
	    filePath			-the filePath of loading SEQ data file
	Output:
	    seqCount  		    -the SEQ count,range:[1-64]
	    triggerSource       -the trigger source data
        intensity           -the intensity data
	...
	Return: 
		...
		failed     -OPT_ERR_READSEQTABLEDATA_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_LoadSeqFile(char* filePath, int* seqCount, int* triggerSource, int* intensity, int* pulseWidth);

/*******************************************************************
	Function:OPTController_CompareSeqTable
	Description:Comparing the  current module of SEQ data with loading file data ,if the return value is 0,
	the loading file data is equal to module file data
	Input:
	controllerHandle		-the handle of controller
	moduleIndex			-the index of the moduleIndex, range:[1-9] (in decimal form)
	Output:
	seqCount  		    -the SEQ count,range:[1-64]
	triggerSource        -the trigger source data
	intensity            -the intensity data
	...
	failed     -OPT_ERR_READSEQTABLEDATA_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_CompareSeqTable(OPTController_Handle controllerHandle, int moduleIndex, char* filePath);

/*******************************************************************
	Function:OPTController_SaveSeqFile
	Description: Saving the SEQ table data to a csv file.
	Input:
	seqCount  		    -the SEQ count,range:[1-64]
	triggerSource       -the trigger source data
	intensity           -the intensity data
	...
	failed     -OPT_ERR_READSEQTABLEDATA_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_SaveSeqFileToCSV(int seqCount, int* triggerSource, int* intensity, int* pulseWidth, char* filePath);

/*******************************************************************
	Function:OPTController_LoadSeqFile
	Description: Loading SEQ table data.
	Input:
	filePath			-the filePath of loading SEQ data file from a csv file.
	Output:
	seqCount  		    -the SEQ count,range:[1-64]
	triggerSource       -the trigger source data
	intensity           -the intensity data
	...
	failed     -OPT_ERR_READSEQTABLEDATA_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_LoadSeqFileFromCSV(char* filePath, int* seqCount, int* triggerSource, int* intensity, int* pulseWidth);
...
/*******************************************************************
	...
	Input:
	   controllerHandle		-the handle of controller
	   moduleIndex			-the index of the moduleIndex, range:[1-8] (in decimal form)
	Output:
	    seqCount  		    -the SEQ count,range:[1-64]
	    triggerSource        -the trigger source data
        intensity            -the intensity data
	...
	Return: 
		...
		failed     -OPT_ERR_READSEQTABLEDATA_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_CompareSeqTableFromCSV(OPTController_Handle controllerHandle, int moduleIndex, char* filePath);
...
/*******************************************************************
	...
	Input:
	    controllerHandle		-the handle of controller
		channelIndex            -the index of the channel, range:[0-36] (in decimal form)
...
/*******************************************************************
	Function: OPTController_SetIPConfigurationBySN
	Description: Modify the IP configuration of the controller which serial number is SN by UDP connection.
	Input:
		SN              	 -the SN of controller
		IP                   -the IP address
		subnetMask           -the subnet mask
	...
	Return: 
	    ...
	    failed     -OPT_ERR_SET_IPCONFIG_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_SetIPConfigurationBySN(char* SN, char* IP, char* subnetMask, char* defaultGateway);
...
/*******************************************************************
	...
	Input:
	    controllerHandle		-the handle of controller
		channelIdx		    	-the index(es) of the channel(s), range:[0-36] (in decimal form),0 for all channels,1-36 for specified channel
...
/*******************************************************************
	...
	Input:
	    controllerHandle		-the handle of controller
		channelIdx		    	-the index(es) of the channel(s), range:[1-36] (in decimal form),1-36 for specified channel
    Output:
	    timeUnit                -the time unit,  range:[0-3],0 for 1us,1 for 10us,2 for 1ms,3 for 100ms
	Return: 
		succeed    -OPT_SUCCEED
...
/*******************************************************************
	Function:OPTController_SetHBTriggerUnit
	Description: set the specified channel's high brightness trigger mode time unit 
	Input:
	    controllerHandle		-the handle of controller
		channelIdx		    	-the index(es) of the channel(s), range:[0-36] (in decimal form),0 for all channels,1-36 for specified channel
		timeUnit                -the time unit, range:[0-2],0 for 1us,1 for 10us,2 for 1ms
   
	Return: 
		succeed    -OPT_SUCCEED
		failed     -OPT_ERR_READ_TIMEUNIT_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_SetHBTriggerUnit(OPTController_Handle controllerHandle, int channelIndex, int timeUnit);

/*******************************************************************
	Function:OPTController_ReadHBTriggerUnit
	Description: Read the specified channel's high brightness trigger mode time unit 
	Input:
	    controllerHandle		-the handle of controller
		channelIdx		    	-the index(es) of the channel(s), range:[1-36] (in decimal form),1-36 for specified channel
    Output:
	...
	Return: 
		succeed    -OPT_SUCCEED
		failed     -OPT_ERR_READ_HB_TIMEUNIT_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_ReadHBTriggerUnit(OPTController_Handle controllerHandle, int channelIndex, int* timeUnit);

/*******************************************************************
	Function:OPTController_SetTriggerDelayUnit
	Description: set the specified channel's trigger delay time unit 
	Input:
	    controllerHandle		-the handle of controller
		channelIdx		    	-the index(es) of the channel(s), range:[0-36] (in decimal form),0 for all channels,1-36 for specified channel
		timeUnit                -the time unit, range:[0-2],0 for 1us,1 for 10us,2 for 1ms
	...
	Return: 
		succeed    -OPT_SUCCEED
		failed     -OPT_ERR_SET_TRIGGERDELAY_TIMEUNIT_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_SetTriggerDelayUnit(OPTController_Handle controllerHandle, int channelIndex, int timeUnit);

/*******************************************************************
	Function:OPTController_ReadTriggerDelayUnit
	Description: Read the specified channel's trigger delay time unit  
	Input:
	    controllerHandle		-the handle of controller
		channelIdx		    	-the index(es) of the channel(s), range:[1-36] (in decimal form),1-36 for specified channel
    Output:
	...
	Return: 
		succeed    -OPT_SUCCEED
		failed     -OPT_ERR_READ_TRIGGERDELAY_TIMEUNIT_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_ReadTriggerDelayUnit(OPTController_Handle controllerHandle, int channelIndex, int* timeUnit);

/*******************************************************************
	Function:OPTController_SetPercentOfBrighteningCurrent
	Description: set the specified channel's brightening current percentage 
	Input:
	    controllerHandle		-the handle of controller
		channelIdx		    	-the index(es) of the channel(s), range:[0-2] (in decimal form),0 for all channels,1-2 for specified channel
		percentage             -the percent of current, range:[101-2000]
   
	Return: 
		succeed    -OPT_SUCCEED
		failed     -OPT_ERR_SET_PERCENT_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_SetPercentOfBrighteningCurrent(OPTController_Handle controllerHandle, int channelIndex, int percentage);

/*******************************************************************
	Function:OPTController_ReadPercentOfBrighteningCurrent
	Description: read the specified channel's brightening current percentage 
	Input:
	    controllerHandle		-the handle of controller
		channelIdx		    	-the index(es) of the channel(s), range:[0-2] (in decimal form),0 for all channels,1-2 for specified channel
	Output:
	...
	Return: 
		succeed    -OPT_SUCCEED
		failed     -OPT_ERR_READ_PERCENT_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_ReadPercentOfBrighteningCurrent(OPTController_Handle controllerHandle, int channelIndex, int* percentage);

/*******************************************************************
	Function:OPTController_SetHBTriggerOutputDutyLimitSwitchState
	Description: set the high brightness trigger output duty cycle limit switch state
	Input:
	    controllerHandle		-the handle of controller
		channelIdx		    	-the index(es) of the channel(s), range:[0-36] (in decimal form),0 for all channels,1-36 for specified channel
		state                   -the state of switch, 0 for Close,1 for Open
   
	Return: 
		succeed    -OPT_SUCCEED
		failed     -OPT_ERR_SET_HB_LIMIT_STATE_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_SetHBTriggerOutputDutyLimitSwitchState(OPTController_Handle controllerHandle, int channelIndex, int state);

/*******************************************************************
	Function:OPTController_ReadHBTriggerOutputDutyLimitSwitchState
	Description: read the high brightness trigger output duty cycle limit switch state
	Input:
	    controllerHandle		-the handle of controller
		channelIdx		    	-the index(es) of the channel(s), range:[0-36] (in decimal form),0 for all channels,1-36 for specified channel
	Output:
	...
	Return: 
		...
		failed     -OPT_ERR_SET_HB_LIMIT_STATE_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_ReadHBTriggerOutputDutyLimitSwitchState(OPTController_Handle controllerHandle, int channelIndex, int* state);

/*******************************************************************
	Function:OPTController_SetHBTriggerOutputDutyRatio
	Description: set the high brightness trigger output duty cycle
	Input:
	    controllerHandle		-the handle of controller
		channelIdx		    	-the index(es) of the channel(s), range:[0-36] (in decimal form),0 for all channels,1-36 for specified channel
		ratio                   -the duty ratio of the output 
   
	Return: 
		succeed    -OPT_SUCCEED
		failed     -OPT_ERR_SET_HB_TRIGGER_OUTPUT_DUTY_RATIO_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_SetHBTriggerOutputDutyRatio(OPTController_Handle controllerHandle, int channelIndex, int ratio);

/*******************************************************************
	Function:OPTController_ReadHBTriggerOutputDutyRatio
	Description: read the high brightness trigger output duty cycle
	Input:
	    controllerHandle		-the handle of controller
		channelIdx		    	-the index(es) of the channel(s), range:[0-36] (in decimal form),0 for all channels,1-36 for specified channel
	Output:
	...
	Return: 
		succeed    -OPT_SUCCEED
		failed     -OPT_ERR_READ_HB_TRIGGER_OUTPUT_DUTY_RATIO_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_ReadHBTriggerOutputDutyRatio(OPTController_Handle controllerHandle, int channelIndex, int* ratio);

/*******************************************************************
	Function:OPTController_SetDiffPresureLimitSwitchState
	Description: set the differential pressure limit switch state
	Input:
	    controllerHandle		-the handle of controller
		channelIdx		    	-the index(es) of the channel(s), range:[0-36] (in decimal form),0 for all channels,1-36 for specified channel
		state                   -the state of switch, 0 for Close,1 for Open
	...
	Return: 
		...
		failed     -OPT_ERR_SET_HB_TRIGGER_OUTPUT_DUTY_RATIO_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_SetDiffPresureLimitSwitchState(OPTController_Handle controllerHandle, int channelIndex, int state);

/*******************************************************************
	Function:OPTController_ReadDiffPresureLimitSwitchState
	Description: read the differential pressure limit switch state
	Input:
	    controllerHandle		-the handle of controller
		channelIdx		    	-the index(es) of the channel(s), range:[0-36] (in decimal form),0 for all channels,1-36 for specified channel
	Output:
	...
	Return: 
		...
		failed     -OPT_ERR_SET_HB_TRIGGER_OUTPUT_DUTY_RATIO_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_ReadDiffPresureLimitSwitchState(OPTController_Handle controllerHandle, int channelIndex, int* state);

/*******************************************************************
	Function:OPTController_SetStartAddress
	Description: set the specified channel's start address index.
	Input:
	controllerHandle		-the handle of controller
	moduleIndex			-the index of the module, range:[1-8],
	1 for channel 1 to channel 4;
	2 for channel 5 to channel 8;
	3 for channel 9 to channel 12;
	4 for channel 13 to channel 16;
	...
	7 for channel 25 to channel 28;
	8 for channel 29 to channel 32;
	startAddrIndex		    -the specified channel's start address index,range:[1-64]
	Return:
	succeed    -OPT_SUCCEED
	failed     -OPT_ERR_SET_START_ADDRESS_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_SetStartAddress(OPTController_Handle controllerHandle, int moduleIndex, int startAddrIndex);

/*******************************************************************
	Function:OPTController_SetEndAddress
	Description: set the specified channel's end address index.
	Input:
	controllerHandle		-the handle of controller
	moduleIndex			-the index of the module, range:[1-8],
	1 for channel 1 to channel 4;
	...
	6 for channel 21 to channel 24;
	7 for channel 25 to channel 28;
	8 for channel 29 to channel 36;
	endAddrIndex		    -the specified channel's end address index,range:[1-64]
	Return:
	succeed    -OPT_SUCCEED
	failed     -OPT_ERR_SET_END_ADDRESS_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_SetEndAddress(OPTController_Handle controllerHandle, int moduleIndex, int endAddrIndex);

/*******************************************************************
	Function:OPTController_SetHardwareResetSwitch
	Description: set the specified channel's hardware reset switch state.
	Input:
	controllerHandle		-the handle of controller
	moduleIndex			-the index of the module, range:[1-8],
	1 for channel 1 to channel 4;
	...
	Return:
	succeed    -OPT_SUCCEED
	failed     -OPT_ERR_SET_HARDWARE_RESET_SWITCH_STATE_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_SetHardwareResetSwitchState(OPTController_Handle controllerHandle, int moduleIndex, int state);

/*******************************************************************
	Function:OPTController_SetAddressSwitch
	Description: set the specified channel's address switch state.
	Input:
	controllerHandle		-the handle of controller
	moduleIndex			-the index of the module, range:[1-8],
	1 for channel 1 to channel 4;
	...
	Return:
	succeed    -OPT_SUCCEED
	failed     -OPT_ERR_SET_ADDRESS_SWITCH_STATE_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_SetAddressSwitchState(OPTController_Handle controllerHandle, int moduleIndex, int state);

/*******************************************************************
	Function: OPTController_ReadProgrammableParameters
	Description: read programmable parameters for the specified channel's.
	Input:
	controllerHandle	-the handle of controller
	moduleIndex			-the index of the module, range:[1-8],
	1 for channel 1 to channel 4;
	...
	7 for channel 25 to channel 28;
	8 for channel 29 to channel 32;
	mode               -the mode of value,range:[0-3]
	0:Read start address value;
	1:Read end address value;
	2:Read hardware reset switch value;
	3:Read address switch value.
	Output(s):
	value    -the obtained value.
	Return:
	succeed    -OPT_SUCCEED
	failed     -OPT_ERR_READ_PROGRAMMABLE_PARAM_FAILED
	*****************************************************************/
long DLL_EXPORT OPTController_ReadProgrammableParameters(OPTController_Handle controllerHandle, int moduleIndex, int mode, int* value);

/********************************************************************
    Function: OPTController_SetDBoardHandleIntensityResultSwitchState
    Description: Set D board handle intensity result switch state
    Input:
        controllerHandle        -the handle of controller
        state                   0: close
                                1: open
    Return: 
        succeed    -OPT_SUCCEED
        failed     -OPT_ERR_SET_DBOARD_HANDLE_INTENSITY_RESULT_FAILED
 ********************************************************************/
long DLL_EXPORT OPTController_SetDBoardHandleIntensityResultSwitchState(OPTController_Handle controllerHandle, int state);

/********************************************************************
    Function: OPT_ERR_READ_DBOARD_HANDLE_INTENSITY_RESULT_FAILED
    Description: Read D board handle intensity result switch state
    Input:
        controllerHandle        -The handle of controller
    Output:
        state                   0: close
    ...
    Return: 
        succeed    -OPT_SUCCEED
        failed     -OPT_ERR_READ_DBOARD_HANDLE_INTENSITY_RESULT_FAILED
 ********************************************************************/
long DLL_EXPORT OPTController_ReadDBoardHandleIntensityResultSwitchState(OPTController_Handle controllerHandle, int* state);

/********************************************************************
    Function: OPTController_SetDOControlSource
    Description: Set DO control source
    Input:
        controllerHandle        -The handle of controller
        channelIndex            0: All Channel
                                [1-36]: Specific channel.
        ctrlSrc                 0:Set by software.
                                1:Trig1.
                                2:Trig2.
                                3:Trig3.
                                4:Trig4.
    Return: 
        succeed    -OPT_SUCCEED
        failed     -OPT_ERR_SET_DO_CONTROL_SOURCE_FAILED
 ********************************************************************/
long DLL_EXPORT OPTController_SetDOControlSource(OPTController_Handle controllerHandle, int channelIndex, int ctrlSrc);

/********************************************************************
    Function: OPTController_ReadDOControlSource
    Description: Read DO control source
    Input:
        controllerHandle        -The handle of controller
        channelIndex            -Specific Channel. Range:[1-36]
    Output:
        ctrlSrc                 0:Set by software.
                                1:Trig1.
                                2:Trig2.
    ...
    Return: 
        succeed    -OPT_SUCCEED
        failed     -OPT_ERR_READ_DO_CONTROL_SOURCE_FAILD
 ********************************************************************/
long DLL_EXPORT OPTController_ReadDOControlSource(OPTController_Handle controllerHandle, int channelIndex, int* ctrlSrc);

/********************************************************************
    Function: OPTController_SetDOControlMode
    Description: Set DO control mode
    Input:
        controllerHandle        -The handle of controller
        channelIndex            0: All Channel
                                [1-36]: Specific channel.
        mode                    0:Follow lighting source.
                                1:Follow pulse width.
    Return: 
        succeed    -OPT_SUCCEED
        failed     -OPT_ERR_SET_DO_CONTROL_MODE_FAILED
 ********************************************************************/
long DLL_EXPORT OPTController_SetDOControlMode(OPTController_Handle controllerHandle, int channelIndex, int mode);

/********************************************************************
    Function: OPTController_ReadDOControlMode
    Description: Read DO control mode
    Input:
        controllerHandle        -The handle of controller
        channelIndex            -Specific Channel. Range:[1-36]
    Output:
    ...
    Return: 
        succeed    -OPT_SUCCEED
        failed     -OPT_ERR_READ_DO_CONTROL_MODE_FAILD
 ********************************************************************/
long DLL_EXPORT OPTController_ReadDOControlMode(OPTController_Handle controllerHandle, int channelIndex, int* mode);

/********************************************************************
    Function: OPTController_SetDOPolarity
    Description: Set DO polarity
    Input:
        controllerHandle        -The handle of controller
        channelIndex            0: All Channel
                                [1-36]: Specific channel.
        polarity                0:Negative follow.
                                1:Positive follow.
    Return: 
        succeed    -OPT_SUCCEED
        failed     -OPT_ERR_SET_DO_POLARITY_FAILED
 ********************************************************************/
long DLL_EXPORT OPTController_SetDOPolarity(OPTController_Handle controllerHandle, int channelIndex, int polarity);

/********************************************************************
    Function: OPTController_ReadDOPolarity
    Description: Read DO polarity
    Input:
        controllerHandle        -The handle of controller
        channelIndex            -Specific Channel. Range:[1-36]
    Output:
    ...
    Return: 
        succeed    -OPT_SUCCEED
        failed     -OPT_ERR_READ_DO_POLARITY_FAILD
 ********************************************************************/
long DLL_EXPORT OPTController_ReadDOPolarity(OPTController_Handle controllerHandle, int channelIndex, int* polarity);

/********************************************************************
    Function: OPTController_SetDOStandbyLevel
    Description: Set DO standby level
    Input:
        controllerHandle        -The handle of controller
        channelIndex            0: All Channel
                                [1-36]: Specific channel.
    ...
    Return: 
        succeed    -OPT_SUCCEED
        failed     -OPT_ERR_SET_DO_STANDBY_LEVEL_FAILED
 ********************************************************************/
long DLL_EXPORT OPTController_SetDOStandbyLevel(OPTController_Handle controllerHandle, int channelIndex, int level);

/********************************************************************
    Function: OPTController_ReadDOStandbyLevel
    Description: Read DO standby level
    Input:
        controllerHandle        -The handle of controller
        channelIndex            -Specific Channel. Range:[1-36]
    Output:
    ...
    Return: 
        succeed    -OPT_SUCCEED
        failed     -OPT_ERR_READ_DO_STANDBY_LEVEL_FAILED
 ********************************************************************/
long DLL_EXPORT OPTController_ReadDOStandbyLevel(OPTController_Handle controllerHandle, int channelIndex, int* level);

/********************************************************************
    Function: OPTController_SetDOSoftwareLevelControl
    Description: Set DO software level control
    Input:
        controllerHandle        -The handle of controller
        channelIndex            0: All Channel
                                [1-36]: Specific channel.
    ...
    Return: 
        succeed    -OPT_SUCCEED
        failed     -OPT_ERR_SET_DO_SOFTWARE_LEVEL_CONTROL_FAILED
 ********************************************************************/
long DLL_EXPORT OPTController_SetDOSoftwareLevelControl(OPTController_Handle controllerHandle, int channelIndex, int level);

/********************************************************************
    Function: OPTController_ReadDOSoftwareLevelControl
    Description: Read DO software level control
    Input:
        controllerHandle        -The handle of controller
        channelIndex            -Specific Channel. Range:[1-36]
    Output:
    ...
    Return: 
        succeed    -OPT_SUCCEED
        failed     -OPT_ERR_READ_DO_SOFTWARE_LEVEL_CONTROL_FAILED
 ********************************************************************/
long DLL_EXPORT OPTController_ReadDOSoftwareLevelControl(OPTController_Handle controllerHandle, int channelIndex, int* level);

/********************************************************************
    Function: OPTController_SetDOMultiSoftwareLevelControl
    Description: Set DO multi software level control
    Input:
        controllerHandle        -The handle of controller
        softwareLevelArray      -DOSoftwareLevelItem struct array
        length                  -The length of softwareLevelArray
    Return: 
        succeed    -OPT_SUCCEED
        failed     -OPT_ERR_SET_DO_SOFTWARE_LEVEL_CONTROL_FAILED
 ********************************************************************/
long DLL_EXPORT OPTController_SetDOMultiSoftwareLevelControl(OPTController_Handle controllerHandle, DOSoftwareLevelItem* softwareLevelArray, int length);

/********************************************************************
    Function: OPTController_ReadDOActualOutputLevel
    Description: Read DO actual output level
    Input:
        controllerHandle        -The handle of controller
        channelIndex            -Specific Channel. Range:[1-36]
    Output:
    ...
    Return: 
        succeed    -OPT_SUCCEED
        failed     -OPT_ERR_READ_DO_ACTUAL_OUTPUT_LEVEL_FAILED
 ********************************************************************/
long DLL_EXPORT OPTController_ReadDOActualOutputLevel(OPTController_Handle controllerHandle, int channelIndex, int* level);

/********************************************************************
    Function: OPTController_SetDODelay
    Description: Set DO delay
    Input:
        controllerHandle        -The handle of controller
        channelIndex            0: All Channel
                                [1-36]: Specific channel.
    ...
    Return: 
        succeed    -OPT_SUCCEED
        failed     -OPT_ERR_SET_DO_DELAY_FAILED
 ********************************************************************/
long DLL_EXPORT OPTController_SetDODelay(OPTController_Handle controllerHandle, int channelIndex, int delay);

/********************************************************************
    Function: OPTController_ReadDODelay
    Description: Read DO delay
    Input:
        controllerHandle        -The handle of controller
        channelIndex            -Specific Channel. Range:[1-36]
    Output:
    ...
    Return: 
        succeed    -OPT_SUCCEED
        failed     -OPT_ERR_READ_DO_DELAY_FAILED
 ********************************************************************/
long DLL_EXPORT OPTController_ReadDODelay(OPTController_Handle controllerHandle, int channelIndex, int* delay);

/********************************************************************
    Function: OPTController_SetDODelayUnit
    Description: Set DO delay unit
    Input:
        controllerHandle        -The handle of controller
        channelIndex            0: All Channel
                                [1-36]: Specific channel.
    ...
    Return: 
        succeed    -OPT_SUCCEED
        failed     -OPT_ERR_SET_DO_DELAY_UNIT_FAILED
 ********************************************************************/
long DLL_EXPORT OPTController_SetDODelayUnit(OPTController_Handle controllerHandle, int channelIndex, int unit);

/********************************************************************
    Function: OPTController_ReadDODelayUnit
    Description: Read DO delay unit
    Input:
        controllerHandle        -The handle of controller
        channelIndex            -Specific Channel. Range:[1-36]
    Output:
    ...
    Return: 
        succeed    -OPT_SUCCEED
        failed     -OPT_ERR_READ_DO_DELAY_UNIT_FAILED
 ********************************************************************/
long DLL_EXPORT OPTController_ReadDODelayUnit(OPTController_Handle controllerHandle, int channelIndex, int* unit);

/********************************************************************
    Function: OPTController_SetDOTriggerWidth
    Description: Set DO trigger width
    Input:
        controllerHandle        -The handle of controller
        channelIndex            0: All Channel
                                [1-36]: Specific channel.
    ...
    Return: 
        succeed    -OPT_SUCCEED
        failed     -OPT_ERR_SET_DO_TRIGGER_WIDTH_FAILED
 ********************************************************************/
long DLL_EXPORT OPTController_SetDOTriggerWidth(OPTController_Handle controllerHandle, int channelIndex, int width);

/********************************************************************
    Function: OPTController_ReadDOTriggerWidth
    Description: Read DO trigger width
    Input:
        controllerHandle        -The handle of controller
        channelIndex            -Specific Channel. Range:[1-36]
    Output:
    ...
    Return: 
        succeed    -OPT_SUCCEED
        failed     -OPT_ERR_READ_DO_TRIGGER_WIDTH_FAILED
 ********************************************************************/
long DLL_EXPORT OPTController_ReadDOTriggerWidth(OPTController_Handle controllerHandle, int channelIndex, int* width);

/********************************************************************
    Function: OPTController_SetDOTriggerWidthUnit
    Description: Set DO trigger widdth unit
    Input:
        controllerHandle        -The handle of controller
        channelIndex            0: All Channel
                                [1-36]: Specific channel.
    ...
    Return: 
        ...
        failed     -OPT_ERR_SET_DO_DELAY_UNIT_FAILED
 ********************************************************************/
long DLL_EXPORT OPTController_SetDOTriggerWidthUnit(OPTController_Handle controllerHandle, int channelIndex, int unit);

/********************************************************************
    Function: OPTController_ReadDOTriggerWidthUnit
    Description: Read DO trigger width unit
    Input:
        controllerHandle        -The handle of controller
        channelIndex            -Specific Channel. Range:[1-36]
    Output:
    ...
    Return: 
        ...
        failed     -OPT_ERR_READ_DO_DELAY_UNIT_FAILED
 ********************************************************************/
long DLL_EXPORT OPTController_ReadDOTriggerWidthUnit(OPTController_Handle controllerHandle, int channelIndex, int* unit);

/********************************************************************
    Function: OPTController_SetUserName
    ...
    Return: 
        succeed    -OPT_SUCCEED
        failed     -OPT_ERR_SET_USER_NAME_FAILED
 ********************************************************************/
long DLL_EXPORT OPTController_SetUserName(OPTController_Handle controllerHandle, char* userName);

/********************************************************************
    Function: OPTController_ReadUserName
    ...
    Return: 
        succeed    -OPT_SUCCEED
        failed     -OPT_ERR_READ_USER_NAME_FAILED
 ********************************************************************/
long DLL_EXPORT OPTController_ReadUserName(OPTController_Handle controllerHandle, char* userName);
};
