    ...
    public:
        ...
        inline bool is_little_big_endianness_swap()
        {
            ...
            else
            {
                ...
                PFM_endianness endianType_ = ((*temp) ^ 0xef == 0 ? PFM_endianness::LITTLE : (*temp) ^ (0xde) == 0 ? PFM_endianness::BIG
                    : PFM_endianness::PERROR);
                // ".pfm" format file specifies that:
                // positive scale means big endianess;
                // negative scale means little endianess.
                return ((PFM_endianness::BIG == endianType_) && (this->endianess < 0.f)) || ((PFM_endianness::LITTLE == endianType_) && (this->endianess > 0.f));
            }
        }
        ...
        static cv::Mat read_pfm(const std::string& filename);
        static Eigen::MatrixXd read_pfm_eigen(const std::string& filename);
        ...
        static void write_pfm(const std::string& filename, cv::Mat& img);
