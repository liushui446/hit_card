...
PointEx rect4th(PointEx a, PointEx b, PointEx c) {
	PointEx d;
	if (fabs(dotMultiply(a, b, c)) < EP_AOI) // 说明c点是直角拐角处 
	{
		d.x = a.x + b.x - c.x;
		d.y = a.y + b.y - c.y;
	}
	if (fabs(dotMultiply(a, c, b)) < EP_AOI) // 说明b点是直角拐角处 
	{
		d.x = a.x + c.x - b.x;
		d.y = a.y + c.y - b.x;
	}
	if (fabs(dotMultiply(c, b, a)) < EP_AOI) // 说明a点是直角拐角处 
	{
		d.x = c.x + b.x - a.x;
		d.y = c.y + b.y - a.y;
	}
...
int circleRelation(PointEx p1, double r1, PointEx p2, double r2) {
	...
	if (fabs(d - r1 - r2) < EP_AOI) // 必须保证前两个if先被判定！ 
		return 2;
